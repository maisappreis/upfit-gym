...
import { globalMethods } from "@/utils/methods";
...
export default {
  ...
  mounted() {
    if (this.revenue && this.revenue.length > 0) {
      ...
      this.monthlyRevenueOrdered = this.sortData(monthlyRevenue);
      this.monthlyExpensesOrdered = this.sortData(monthlyExpenses);
